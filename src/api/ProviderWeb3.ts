...
import Web3Utils, { Unit } from 'web3-utils'

import { netIdToName, netIdToWebsocket, windowLoaded } from 'utils'

import { Account, Web3AppAPI } from 'types'
...
const setupWeb3Watchdog = async (web3: any) => {
...
async function init() {
  ...
  const getBalance = (account: Account) => web3.eth.getBalance(account)
  ...
  */
  ...
  /**
   * getNetwork
   * @returns { Promise<"Mainnet" | "Morden" | "Ropsten" | "Rinkeby" | "Kovan" | "No network detected" | "Local Network"> } network NAME
   */
   ...
   * @returns {string}
   */
  const toWei = (amount: string, x?: Unit) => utils.toWei(amount, x)
  const fromWei = (amount: string, x?: Unit) => utils.fromWei(amount, x)
  const toBN = (amount: string | number) => utils.toBN(amount)
